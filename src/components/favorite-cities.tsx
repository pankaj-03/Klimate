...
const FavoriteCities = () => {
  ...
  return (
    <>
      <h1 className="text-xl font-bold tracking-tight text-white">Favorites</h1>
      <ScrollArea className="w-full pb-4">
          <div className="flex gap-4">
            {favorites.map((city)=>{
                return (<FavoriteCityTablet key={city.id}
                                               {...city} 
...
function FavoriteCityTablet({id , name , lat , lon , onRemove}: FavoriteCityTabletProps){
               ...
               return (
                ...
                className="relative flex min-w-[250px] cursor-pointer items-center gap-3 rounded-lg border bg-card p-4 pr-8 shadow-sm transition-all hover:shadow-md">
                   ...
                   {isLoading?(
                    <div className="flex h-8 items-center justify-content">
